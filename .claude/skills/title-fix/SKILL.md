...
before the first one, nothing after the last one. That includes any narration
...
6. **Cross-session reference, when one is explicit, is present and short**: if this thread's
   ...
   another session (not "might be relevant" but actually said, e.g. the UUID was quoted in a
...
Join tersely as `anchor: work`. Don't restate the whole thread history; the
...
Format: the 8-hex prefix of the other session's UUID, in the work slot, in prose, as in
